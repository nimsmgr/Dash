...
class Player extends SpriteGroupComponent<PlayerState>
  ...
  void moveLeft() {
    ...
    if (isWearingHat) {
      ...
      current = PlayerState.shieldLeft;
    } else if (!hasPowerup) {
  ...
  bool get isShield =>
      current == PlayerState.shieldLeft ||
      current == PlayerState.shieldRight ||
      current == PlayerState.shieldCenter;
  @override
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    super.onCollision(intersectionPoints, other);
    if (other is EnemyPlatform && !isInvincible && !isShield) {
      gameRef.onLose();
      return;
    }
    ...
    if (!hasPowerup && other is Rocket) {
      ...
      jump(specialJumpSpeed: jumpSpeed * other.jumpSpeedMultiplier);
      return;
    } else if (!hasPowerup && other is Shield) {
      if (current == PlayerState.center) current = PlayerState.shieldCenter;
      if (current == PlayerState.left) current = PlayerState.shieldLeft;
      if (current == PlayerState.right) current = PlayerState.shieldRight;
      other.removeFromParent();
      // _removePowerupAfterTime(other.activeLengthInMS);
      jump(specialJumpSpeed: jumpSpeed * other.jumpSpeedMultiplier);
      return;
  ...
  Future<void> _loadCharacterSprites() async {
    ...
    final shieldCenter =
        await gameRef.loadSprite('game/${character.name}_shield_center.png');
    final shieldLeft =
        await gameRef.loadSprite('game/${character.name}_shield_left.png');
    final shieldRight =
        await gameRef.loadSprite('game/${character.name}_shield_right.png');
    ...
    sprites = <PlayerState, Sprite>{
      ...
      PlayerState.shieldCenter: shieldCenter,
      PlayerState.shieldRight: shieldRight,
      PlayerState.shieldLeft: shieldLeft,
    };
  }
}
